...
import { mockedFeedback, mockedFeedbackBranco, mockedStudentLogin, mockedTeacherLogin } from "../../mocks"
...
let connection: DataSource
    ...
    test('POST /feedback - tentando criar um feedback sendo um professor', async() => { 
        ...
        const response = await request(app).post('/feedback').set("Authorization", `Bearer ${adminLoginResponse.body.token}`).send(mockedFeedback)
    ...
    test('POST /feedback - tentando criar um feedback em branco', async() => { 
        const adminLoginResponse = await request(app).post("/login").send(mockedTeacherLogin);
        const response = await request(app).post('/feedback').set("Authorization", `Bearer ${adminLoginResponse.body.token}`).send(mockedFeedbackBranco)
    ...
    test('DELETE /feedback - tentando deleta um feedback sendo um professor', async() => { 
        ...
        const adminLoginResponse = await request(app).post("/login").send(mockedTeacherLogin);
        const UserTobeDeleted = await request(app).get('/feedback').set("Authorization", `Bearer ${adminLoginResponse.body.token}`)
